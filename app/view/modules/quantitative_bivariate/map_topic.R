...
box::use(
  ...
  app/logic/quantitative_bivariate/gender_inf_logic,
  app/logic/functions
)
...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$barplot <- renderPlotly({
      functions$generate_groupedbarplot(gender_inf_logic$gender_otherforms,"Geschlecht","Diskriminierung")
    })

    output$piechart <- renderPlotly({
      functions$generate_table(gender_inf_logic$gender_otherforms,"Geschlecht", "Discrimination")
    })
