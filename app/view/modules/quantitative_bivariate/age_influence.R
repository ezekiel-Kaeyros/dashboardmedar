...
box::use(
    ...
    app/logic/quantitative_bivariate/age_inf_disc,
    app/logic/functions
)
...
ui <- function(id) {
  ns <- NS(id)
  cards$card_ui("Häufigkeit der verschiedenen Ursachen von Einflüssen nach Altersgruppen",
...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$barplot <- renderPlotly({
      functions$generate_groupedbarplot(age_inf_disc$table_age_inf,"Altersgruppe","Ursachen der Diskriminierung")
    })

    output$piechart <- renderPlotly({
      functions$generate_table(age_inf_disc$table_age_inf,"Altersgruppe", "Ursachen der Diskriminierung")
    })
