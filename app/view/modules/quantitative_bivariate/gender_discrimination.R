...
box::use(
  ...
  app/logic/quantitative_bivariate/gender_disc,
  app/logic/functions
)
...
ui <- function(id) {
  ns <- NS(id)
  cards$card_ui("Häufigkeit der verschiedenen Ursachen von Diskriminierung nach Geschlecht",
...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$barplot <- renderPlotly({
      functions$generate_groupedbarplot(gender_disc$table_gender_disc,"Geschlecht","Ursachen der Diskriminierung")
    })

    output$piechart <- renderPlotly({
      functions$generate_table(gender_disc$table_gender_disc,"Geschlecht", "Ursachen der Diskriminierung")
    })
